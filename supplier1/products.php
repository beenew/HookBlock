...
public function getitem(string $userID = Null){
    ...
    if ($this->database->getReference($this->dbname)->getSnapshot()->hasChild($this->supplierid)){
        return $this->database->getReference($this->dbname)->getChild($this->supplierid)->getValue();
    } else {
        return FALSE;
    }
}
...
var_dump($users->getitem("1564"));

foreach($users->getitem("1564")!=Null){
